...
async def _get_scope(
    ...
    department_ids = [str(d) for d in employee.department_ids]
...
def _make_identity(scope: dict):
    ...
    apply_scope_filter only reads allowed_source_ids, allowed_knowledge_types
    and project_source_ids. Chat does not resolve workspace-granted source IDs,
    so project_source_ids is empty here — source-listing tools fall back to KT
    and explicit grants. Wiki tools still get full workspace scoping via
    scope["project_ids"].
    """
    class _Identity:
...
async def execute_tool(
    ...
    dept_uuids = [_uuid.UUID(d) for d in scope["department_ids"]]
